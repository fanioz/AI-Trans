...
function AyStar::InitializePath(sources, goals, ignored_tiles = [])
{
	...
	foreach (node in sources) {
		if (typeof(node) == "array") {
			...
			this._open.Insert(new_path, new_path.GetCost() + this._estimate_callback.call(this._pf_instance, node[0], node[1], goals));
		} else {
	...
	while (this._open.Count() > 0 && (iterations == -1 || iterations-- > 0)) {
		...
		if (this._closed.HasItem(cur_tile)) {
			...
			while (scan_path != null) {
				if (scan_path.GetTile() == cur_tile) {
					if (!this._check_direction_callback.call(this._pf_instance, cur_tile, scan_path.GetDirection(), path.GetDirection())) {
						mismatch = true;
						break;
					}
				}
		...
		foreach (goal in this._goals) {
			if (typeof(goal) == "array") {
				if (cur_tile == goal[0]) {
					local neighbours = this._neighbours_callback.call(this._pf_instance, path, cur_tile);
		...
		foreach (node in neighbours) {
			...
			this._open.Insert(new_path, new_path.GetCost() + this._estimate_callback.call(this._pf_instance, node[0], node[1], this._goals));
		}
	}
...
class AyStar.Path
{
	...
	constructor(old_path, new_tile, new_direction, cost_callback, pf_instance)
	{
		...
		this._cost = cost_callback.call(pf_instance, old_path, new_tile, new_direction);
		if (old_path == null) {
			this._length = 0;
		} else {
