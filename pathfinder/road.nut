...
function RoadPF::InitializePath(sources, goals, ignored_tiles = [], max_length_multiplier = 0, max_length_offset = 10000)
{
  local nsources = [];
...
function RoadPF::RegisterCostCallback(callback, ...)
{
    local args = [];
...
function RoadPF::_Cost(path, new_tile, new_direction)
{
...
function RoadPF::_Estimate(cur_tile, cur_direction, goal_tiles)
{
    /* As estimate we multiply the lowest possible cost for a single tile with
    * with the minimum number of tiles we need to traverse. */
    return AIMap.DistanceManhattan(cur_tile, this._goal_estimate_tile) * /* this._cost_no_existing_road*/ this._cost_tile * this._estimate_multiplier;
}

function RoadPF::_Neighbours(path, cur_node)
{
    ...
    if ((AIBridge.IsBridgeTile(cur_node) || AITunnel.IsTunnelTile(cur_node)) &&
        ...
        if (AIRoad.AreRoadTilesConnected(cur_node, next_tile) || AITile.IsBuildable(next_tile) ||
            AIRoad.IsRoadTile(next_tile) || (this._allow_demolition && AITile.DemolishTile(next_tile))) {
        ...
        foreach (offset in offsets) {
            ...
            * 1) There already is a connections between the current tile and the next tile.*/
            if (AIRoad.AreRoadTilesConnected(cur_node, next_tile)) tiles.push([next_tile, this._GetDirection(cur_node, next_tile, false)]);
            /* 2) We can build a road to the next tile. */
            if ((AITile.IsBuildable(next_tile) ||
                AIRoad.IsRoadTile(next_tile) ||
                can_demolish) &&
                (path.GetParent() == null ||
                AIRoad.CanBuildConnectedRoadPartsHere(cur_node, path.GetParent().GetTile(), next_tile) > 0) &&
                ((AIRoad.BuildRoad(cur_node, next_tile) ||
                (AIError.GetLastError() == AIError.ERR_VEHICLE_IN_THE_WAY &&
                (path.GetParent() == null ||
                AIRoad.CanBuildConnectedRoadPartsHere(cur_node, path.GetParent().GetTile(), next_tile) > 0))) ||
                can_demolish)) tiles.push([next_tile, this._GetDirection(cur_node, next_tile, false)]);
            /* 3) The next tile is the entrance of a tunnel / bridge in the correct direction. */
            if (this._CheckTunnelBridge(cur_node, next_tile)) {
                tiles.push([next_tile, this._GetDirection(cur_node, next_tile, false)]);
            }
        }
...
function RoadPF::_CheckDirection(tile, existing_direction, new_direction)
{
  return false;
}
